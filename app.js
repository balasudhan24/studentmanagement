var students = [
    ...
    for (var i = 0; i < students.length; i++) {
        main.innerHTML += `
            <tr>
                ...
                <td>${students[i].feedback}</td> <!-- Displaying feedback -->
  ...
  function search() {
    var found = false;
    for (var i = 0; i < students.length; i++) { // Declare 'i' variable
        if (searched.value.toLowerCase() == students[i].name.toLowerCase()) {
            found = true;
            Swal.fire({
                title: `Name: ${students[i].name}`,
                text: ` Maths: ${students[i].math} | English: ${students[i].eng} | Urdu: ${students[i].urd} | Science ${students[i].sci} | Total: ${students[i].math + students[i].eng + students[i].urd + students[i].sci} | Percentage: ${((students[i].math + students[i].eng + students[i].urd + students[i].sci) * 100 / 400).toFixed(2)}% | Feedback: ${students[i].feedback}`,
                icon: 'success',
  ...
  function newStudent() {
    Swal.fire({
        ...
        '<input id="swal-input7" class="swal2-input" placeholder="Feedback">', // Adding input for Feedback
        focusConfirm: false,
        preConfirm: () => {
            ...
            const feedback = document.getElementById('swal-input7').value; // Getting feedback value
            ...
            return [name, rollNumber, math, eng, urd, sci, feedback];
        }
    }).then((result) => {
        if (result.isConfirmed) {
            ...
            const student = {
                ...
                feedback: formValues[6] // Assigning feedback value
            };
